...
const ChatWindow = ({ selectedUser }) => {
      ...
      const fetchMessages = async () => {
        try {
          const response = await axios.get(
            `http://chat.localhost/messages/admin/${selectedUser.username}`
          );
  ...
  useEffect(() => {
    if (selectedUser) {
      ws.current = new WebSocket(
        `ws://localhost:8081/ws/admin/${selectedUser.username}?token=${localStorage.getItem("accessToken")}`
      );
  ...
  const sendMessage = () => {
    if (inputMessage.trim() && ws.current) {
      const messageData = {
        sender: "admin", // Отправляем от имени администратора
        recipient: selectedUser.username,
        text: inputMessage,
      };

      ws.current.send(JSON.stringify(messageData));
      setMessages((prev) => [...prev, { sender: "admin", content: inputMessage }]);
      setInputMessage("");
    }
  };
